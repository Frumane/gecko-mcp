 ...
 *  read the response back. Optional GECKO_MCP_ALLOW_DOMAINS restricts to a
 *  domain allowlist. GECKO_MCP_ALLOW_PRIVILEGED_URLS=1 lifts scheme+host gates. */
export function assertNavigableUrl(url: string): void {
  const bypass = envCfg("ALLOW_PRIVILEGED_URLS") === "1";
  ...
  if (scheme !== "http" && scheme !== "https") {
    ...
    throw new Error(
      `Refusing to open "${url}" — only http(s) URLs are allowed by default ` +
        ...
        `Set GECKO_MCP_ALLOW_PRIVILEGED_URLS=1 to allow other schemes.`,
    );
  }
  let host: string;
  try {
  ...
  if (!bypass && isInternalHost(host)) {
    throw new Error(
      `Refusing to navigate to internal/loopback host "${host}" — this could reach ` +
        ...
        `Set GECKO_MCP_ALLOW_PRIVILEGED_URLS=1 to override.`,
    );
  }
  const allow = envCfg("ALLOW_DOMAINS");
  if (allow) {
    ...
    if (!ok) {
      throw new Error(
        `Refusing to navigate to "${host}" — not in GECKO_MCP_ALLOW_DOMAINS allowlist.`,
      );
    }
  }
}

/** If GECKO_MCP_ALLOW_UPLOAD_DIRS is set (';'-separated on Windows), uploads are
...
export function assertUploadAllowed(filePath: string): string {
  ...
  const allow = envCfg("ALLOW_UPLOAD_DIRS");
  ...
  if (!ok) {
    throw new Error(
      `Upload of "${resolved}" blocked — outside GECKO_MCP_ALLOW_UPLOAD_DIRS. ` +
